...
cmake -H$SRC_DIR -B$BUILD_DIR -GNinja -DCMAKE_C_COMPILER=clang-6.0 -DCMAKE_CXX_COMPILER=clang++-6.0 -DCMAKE_VERBOSE_MAKEFILE=true -DCMAKE_BUILD_TYPE=RelWithDebInfo -DHUNTER_ENABLED=ON -DHUNTER_BUILD_SHARED_LIBS=OFF -DHUNTER_STATUS_DEBUG=ON -DENABLE_PDF=ON -DENABLE_DATA_MYSQL=OFF -DENABLE_TESTS=ON -DENABLE_SAMPLES=ON -DPOCO_UNBUNDLED=ON && \
